...
const TechnicalComparison = () => {
  ...
  const isTechnicalClaimsFinalized =
    currentApplicationRejections?.isTechnicalComparisonFinalized;
  const isTechnicalAmendmentClaimsLoading =
    currentApplicationRejections?.isTechnicalComparisonAmendmentClaimsLoading;
  ...
  const handleFinalizeConfirmation = async () => {
    ...
    if (!docketData?.technicalData?.amendedClaim || !isTechnicalClaimsAmended) {
      return toast.error("Please amend the claims first");
    } else if (isTechnicalClaimsFinalized) {
      return;
    }
  ...
  const handleAmendClaimsClick = async (e) => {
    ...
    dispatch(
      setApplicationRejections({
        rejectionId: activeDocketId,
        name: "isTechnicalComparisonAmendmentClaimsLoading",
        value: true,
      })
    );
        ...
        <div className="flex flex-col xl:flex-row gap-4 space-y-6 xl:space-y-0 xl:h-[600px] p-3 sm:p-0">
          ...
          <div className="flex items-center justify-center w-full xl:w-8 h-full">
            <div className="inline-block relative">
              <button
                className={`h-10 w-10 rounded-full bg-[#3586cb] flex items-center justify-center shadow-sm text-white tooltip-trigger ${
            ...
            >
              <div className="h-full bg-gray-50 rounded border border-gray-200 py-3 px-5 overflow-y-auto">
                {isTechnicalAmendmentClaimsLoading ? (
                  ...
                  docketData?.technicalData?.rejectedClaim ? (
